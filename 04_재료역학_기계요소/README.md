...
![ANSYS 구조해석 결과](assets/ansys_result.png)

> **한 줄 소개(문제 중심)**: 부재가 하중을 견디는지 판단하기 위해, 재료역학 이론(굽힘·단면 특성)을 ANSYS 유한요소해석으로 응력·변형까지 검증한 프로젝트.

### 📌 프로젝트 개요 (강의 템플릿)
| 항목 | 내용 |
|------|------|
| 문제 배경 | 손계산만으로는 복잡 형상·하중의 응력 분포·최대 변형 위치를 알기 어려움 |
| 해결 목표 | 스프링·발사체 등 부재의 등가응력(von-Mises)·변형을 FEM으로 산출, 이론과 대조 |
| 기간 / 형태 | 2024-1 / 과목 과제 (ANSYS Workbench 다수 케이스) |
| 역할·기여 | 개인(형상·하중·메시·해석·리포트) |
| 기술 스택 | ANSYS Workbench(Static Structural), DesignModeler, 재료역학 이론 |
| 기술 선택 이유 | 정적 하중 하 응력·변형 분포 확인엔 **선형 정적 구조해석(FEM)** 이 적합 / 이론값(σ=M/Z)으로 결과 타당성 교차검증 |

### 🧩 문제 → 영향 → 해결 → 결과
- **문제**: 부재의 최대 응력·변형 위치와 크기 파악
- **영향**: 과소설계 시 파손, 과대설계 시 중량·비용 증가
- **해결**: 형상→메시→하중·구속→Static Structural 해석으로 등가응력·전 변형량 컨투어 산출
- **결과(지표)**: 케이스별 von-Mises 최대 응력·Total Deformation 도출, 자동 리포트(`SPRING_report.html`) 생성, 이론 굽힘응력과 경향 일치 확인

### 💡 배운 점 · 향후 개선
- **배운 점**: 메시 밀도·경계조건이 결과에 미치는 영향, 이론-FEM 교차검증의 중요성
- **향후 개선**: 메시 수렴성(convergence) 분석 추가, 안전계수 기반 설계 판정 명시
...
| 모델 | 결과 유형 | 최대 | 최소 | 단위 |
|------|-----------|------|------|------|
| 사각 보 | 등가응력 (von-Mises) | **1159.9** | 372.05 | MPa |
| 노치 보 | 등가 교번응력 (Equivalent Alternating) | **1339.7** | 187.69 | MPa |

- 값은 ANSYS **2024 R1 Student** 결과 컨투어 범례에서 판독(`ANSYS_결과이미지_01.png`, `22.png`).
- 재료 **Structural Steel**(항복 ~482 MPa, 인장 ~538 MPa) 대비 최대 응력이 높은 구간은 **응력 집중부** — 메시 세분화·하중/구속 재검토 대상으로 식별.
- 전체 응력·변형 필드 데이터는 결과 파일 `file.rst`(바이너리)에 저장되어 ANSYS Mechanical에서 확인 가능. 자동 리포트는 `SPRING_report.html` 참고.
