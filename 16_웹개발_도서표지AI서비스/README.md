...
### 🧩 문제 → 영향 → 해결 → 결과 (강의 템플릿)
- **문제**: 작가/소상공인이 도서 표지 제작에 시간·비용 부담
- **영향**: 표지 품질이 노출·구매에 영향을 주나 디자인 외주는 비쌈
- **해결**: 도서 내용 기반 구조화 프롬프트로 OpenAI GPT Image 호출 → 표지 자동 생성, CRUD·TTS 통합
- **결과**: `b64_json` 응답을 Data URL로 변환·렌더링 후 `PATCH /books/:id`로 영구 저장하는 end-to-end 플로우 구현
- **기술 선택 이유**: 빠른 SPA·생태계로 **React+Vite** / 백엔드 없이 REST 흉내로 **json-server** Mock(추후 Spring Boot 교체 전제) / 이미지 생성은 **OpenAI GPT Image**, 키는 하드코딩 대신 **UI 입력**으로 보안 처리
- **배운 점·향후 개선**: 외부 API 연동(로딩·에러·비용 안내) 설계 / 실제 백엔드(Spring Boot)·인증·이미지 캐싱으로 확장 예정
