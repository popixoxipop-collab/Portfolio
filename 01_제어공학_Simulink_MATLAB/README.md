...
> **한 줄 소개(문제 중심)**: 노이즈가 섞인 측정값에서 상태를 추정하고 제어하기 위해, Kalman Filter와 제어 루프를 Simulink로 직접 구현·검증한 학습 프로젝트.
> **재사용 가능한 역량**: 상태 추정(KF) 모델링, 블록 다이어그램 설계, 수동(MATLAB) vs 툴(Simulink) **결과 교차검증** 능력.
